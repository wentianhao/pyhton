...
def getPage(key_words):
    ...
    browser = webdriver.PhantomJS('D:/anaconda/phantomjs-2.1.1-windows/bin/phantomjs') # 此处添加phantomjs所在位置
    browser.get(url)
    ...
    html = urllib2.Request(b) # 获取网页源代码
    opener = urllib2.build_opener()
    html = opener.open(html).read() # 读出网页源码内容
    typer = chardet.detect(html)['encoding'] # 网页源码编码格式
    html = html.decode(typer,'ignore') # 解码
    soup = bs(''.join(html))
    a = soup('div',{'class':'l_v2'})[0]
...
def getcontents1(url):
    try:
        html = urllib2.Request(url)  # 获取网页源代码
        opener = urllib2.build_opener()
        html = opener.open(html).read()  # 读出网页源码内容
        typer = chardet.detect(html)['encoding']  # 网页源码编码格式
        data = html.decode(typer, 'ignore')
        soup = bs(data)
        title = soup('title')[0].get_text().split('|')[0]
        description0 = soup('meta',{'name':'description'})
        if len(description0)>0:
            description = str(description0[0]).split('"')[1]  # 获取描述内容
        else:
            description = ''
        ...
        if len(keyword1)>0:
            keyword = str(keyword1[0]).split('"')[1]
        else:
            keyword = ','.join(ContentAnalysis(contentnew)[1])[0]
...
def sent_tokenizer(texts):
    ...
    text1 = texts.decode('utf-8')
    # 一句为一个字符存储，如：'我是四川人，但我在昆明'切分为'我是四川人','但我在昆明'
    for j in range(len(text1)):
        if j != len(text1)-1:
            if text1[j] in punt_list and text1[j+1] not in punt_list: # 检查标点符号下一个字符是否还是标点
                sentences.append(text1[start2:j+1]) # 当前标点符号位置
                start2 = j + 1 # start标记到下一句的开头
        else:
            sentences.append((text1[start2:j])) #最后一句话加入
    return sentences
...
if __name__ == '__main__':
    key_words = raw_input('请输入关键字：')
    typer1 = chardet.detect(key_words)['encoding']
    key_words = key_words.decode(typer1, 'ignore')
