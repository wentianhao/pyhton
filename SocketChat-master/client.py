...
class ClientThread(QThread):
    ...
    output1_signal = pyqtSignal('PyQt_PyObject')
    output2_signal = pyqtSignal('PyQt_PyObject')
    ...
    def run(self):
        while True:
            ...
            self.output1_signal.emit(cons)
            self.output2_signal.emit(cons)
...
class ClientWindow(QtWidgets.QMainWindow, chat.Ui_MainWindow):
    ...
    def send(self):
        if self.thread is not None and self.thread.isRunning():
            ...
            self.socket.send(json.dumps({'private_mode': private_mode, 'msg': text}).encode())
    ...
    def connect_server(self):
        if self.thread is not None and self.thread.isRunning():
            ...
            '''
            self.output_signal.connect(self.print_msg)
            self.output1_signal.connect(self.print_cons)
            self.output2_signal.connect(self.private_con)
