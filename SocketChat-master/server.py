        ...
        while True:
            ...
            if mode in ['', 'None']:
                ...
                for client in self.cons_list:
                    client.send(self.msg_build(text, False).encode())
            else:
                # private mode
                ...
                for i in range(len(self.cons_list)):
                    if self.cmpT(self.addr_list[i], (ip, int(port))):
                        ...
                        self.cons_list[i].send(self.msg_build(text, True).encode())
                        # display msg to sender
                        if self.cons_list[i] is not self.client:
                            self.client.send(self.msg_build(text, True).encode())
                        break
